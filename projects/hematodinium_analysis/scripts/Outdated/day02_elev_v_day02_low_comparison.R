...
# Analysis: Day 0/2 elevated temp vs. Day 0/2 lowered temperature
...
resLFC <- lfcShrink(deseq2.dds, coef = "temp_Low_vs_Elev", 
                    type = "apeglm")
resLFC
...
plotMA(resLFC, ylim = c(-10, 10), main = "apeglm")
...
plot(deseq2_tmp$baseMean, deseq2_tmp$log2FoldChange, pch = 20,
     ...
     main = "Elevated & Low-Temp (padj <= 0.005)",
...
write.table(deseq2_tmp.sig, "Elev_vsLow_DEGlist.txt",
            row.names = TRUE, col.names = FALSE, quote = FALSE,
            sep = "\t")
write.table(deseq2_tmp.sig, "Elev_vsLow_DEGlist_wcols.txt",
