...
# Analysis: Day 0/2 ambient temp vs. Day 0/2 lowered temperature
...
deseq2.colData <- data.frame(temp = factor(c("Amb", "Amb", "Amb",
                                             "Amb", "Amb", "Amb",
                                             "Low", "Low")),
...
resLFC <- lfcShrink(deseq2.dds, coef = "temp_Low_vs_Amb", 
                    type = "apeglm")
resLFC
...
plot(deseq2_tmp$baseMean, deseq2_tmp$log2FoldChange, pch = 20,
     ...
     main = "Ambient & Low-Temp (padj <= 0.005)",
...
write.table(deseq2_tmp.sig, "Amb_vsLow_DEGlist.txt",
            row.names = TRUE, col.names = FALSE, quote = FALSE,
            sep = "\t")
write.table(deseq2_tmp.sig, "Amb_vsLow_DEGlist_wcols.txt",
