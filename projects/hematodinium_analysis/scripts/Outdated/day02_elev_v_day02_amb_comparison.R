...
# Analysis: Day 0/2 elevated temp vs. Day 0/2 ambient temperature
...
deseq2.colData <- data.frame(temp = factor(c("Elev", "Elev", "Elev",
                                             "Elev", "Elev", "Elev",
                                             "Amb", "Amb", "Amb", "Amb", 
                                             "Amb", "Amb")),
...
resLFC <- lfcShrink(deseq2.dds, coef = "temp_Elev_vs_Amb", 
                    type = "apeglm")
resLFC
...
plotMA(deseq2.res, ylim = c(-30, 30))
# Plot of full results, shrunken
plotMA(resLFC, ylim = c(-15, 15), main = "apeglm")
# Plot of res05 results, not shrunken
plotMA(deseq_res05, ylim = c(-30, 30))
...
plot(deseq2_tmp$baseMean, deseq2_tmp$log2FoldChange, pch = 20,
     ...
     main = "Elevated & Ambient-Temp (padj <= 0.005)",
...
write.table(deseq2_tmp.sig, "Elev_vsAmb_DEGlist.txt",
            row.names = TRUE, col.names = FALSE, quote = FALSE,
            sep = "\t")
write.table(deseq2_tmp.sig, "Elev_vsAmb_DEGlist_wcols.txt",
